...
const SettingsModal = ({}, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="fixed inset-0 z-50 px-6 pt-11.5 pb-17 text-mirage md:p-0"
    >
      <div
        onClick={() => uiDispatch({ type: "closeSettingsModal" })}
        className="absolute inset-0 z-10 bg-mirage bg-mirage bg-opacity-50"
      ></div>
      <div className="modal-body relative z-20 mx-auto w-full max-w-[540px] rounded-[25px] bg-white md:absolute md:left-0 md:right-0 md:top-1/4">
        <header className="flex items-center justify-between border-b border-solid border-lilac p-6">
          ...
          <button onClick={() => uiDispatch({ type: "closeSettingsModal" })}>
            <CloseIcon />
